    ...
    try {
      if (selectedItems?.length === 0) {
        Toast.showWithGravity(
          'Select atleast one delivery item',
          Toast.LONG,
          Toast.BOTTOM,
        );
        return;
    ...
    <View style={styles.container}>
      ...
      >
        <Container>
          ...
          />
          <CustomText style={styles.title2}>
            {t('request.more_details')}
          </CustomText>
          <View style={styles.detailsItemView}>
            <CustomText style={styles.detailsLabel}>
              {t('request.urgency_level')}
            </CustomText>
            <CustomText style={[styles.detailsValue, styles.uppercaseTxt]}>
              {requestDetails?.priority}
            </CustomText>
          </View>
          <View style={styles.detailsItemView}>
            <CustomText style={styles.detailsLabel}>
              {t('request.temperature_requirement')}
            </CustomText>
            <CustomText style={styles.detailsValue}>
              {requestDetails?.temperature_requirement || 'N/A'}
            </CustomText>
          </View>
          <View style={styles.detailsItemView}>
            <CustomText style={styles.detailsLabel}>
              {t('request.vehicle_requirements')}
            </CustomText>
            <CustomText style={[styles.detailsValue, styles.capitalizeTxt]}>
              {requestDetails?.vehicle_requirements || 'N/A'}
            </CustomText>
          </View>
          <View style={styles.detailsItemView}>
            <CustomText style={styles.detailsLabel}>
              {t('request.number_of_bags')}
            </CustomText>
            <CustomText style={styles.detailsValue}>
              {requestDetails?.container_count}
            </CustomText>
          </View>
          <CustomText style={styles.title3}>
            {t('route.fill_in_the_details')}
          </CustomText>
          <Formik
            innerRef={formikRef}
            ...
            {({ handleChange, handleSubmit, values, errors, touched }) => (
              <>
                <Input
                  ...
                  autoCapitalize="none"
                  error={
                    touched.temperature_reading
                      ? errors.temperature_reading
                      : undefined
                  }
                  returnKeyType="next"
